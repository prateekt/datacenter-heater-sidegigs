<p align="center">
  ...
  <em>Job 1: power AI. Side gig: put that exhaust to work — DAC, algae, shelter showers, plastic recycling, and a 15,000-instrument GPU fan orchestra inspired by the Pickaso Rotary Bow.</em>
</p>
...
> **In one sentence:** Data centers are giant heaters. **Job 1** is powering AI; the **side gig** is routing that exhaust to carbon capture, algae, shelter showers, and more — **before** it's thrown away. **Long-term north star:** drive **uncompensated** campus externalities toward **zero** by colocating recovery with compute ([details](#north-star-zero-negative-externality)).
...
|---|----------------------------------------|-----------------------------------|
| **Purpose** | Power AI training and inference | Use exhaust **before** it's wasted |
| **Typical fate** | Heat dumped to ambient | DAC, algae, showers, pools, fisheries, plastic recycling |
| **Question** | How much rack power for compute? | How much **GWh/yr** can that heat deliver? |
| **Grid assumption** | Doesn't matter — heat exists either way | Same — we lead with **thermodynamics**, not grid mix |

The name **Side Gig** is deliberate: powering AI still produces unavoidable exhaust heat. We're asking what else that heat could do on the way out.
...
*Auto-generated results for **Data Center Heater Side Gig** — **job 1** powers AI; the **side gig** routes that exhaust to DAC, algae, shelter showers, and more before dissipation. We quantify MW, GWh/yr, and temperature grades (grid-agnostic). Grid-dependent carbon accounting is in the [appendix](#appendix-grid-dependent-carbon-scenario).*

### Thesis

> Data centers are giant heaters. **Job 1** is powering AI; the **side gig** is putting that exhaust to work before it's thrown away. We do not assume clean electricity — we quantify the **output-side thermodynamic potential**: how much heat is produced, what temperatures are available, and which downstream loads can use it.
...
The same **~33.8 MW** waste-heat stream can be routed to **DAC**, **heated pools**, **aquaculture raceways**, **algae**, **plastic recycling**, or **shelter hot showers** (MVP: **one valve path at a time** — robot `priority` in each YAML). **Routed MWh** comes from the simulator; **pools / fisheries / algae lines are zero** when that load is not in the priority list (e.g. `nvidia_us_expansion.yaml` sends everything to DAC). **Homes** and **hot showers** are *hypothetical redirects* of the same total MWh — not simultaneous loads.

| Priority scenario | Robot order | Routed MWh/yr (pool · fish · algae · plastic · DAC) | Net CO₂e (t/yr) |
|-------------------|-------------|-----------------------------------------------------|-----------------|
| DAC priority (climate) | carbon_capture → algae → buffer | **70,918** (0 · 0 · 0 · 0 · 70918) | 37,776 |
| Community heat (pools + fisheries) | pool → aquaculture → algae → carbon_capture → buffer | **6,510** (97 · 104 · 3409 · 0 · 2900) | 1,721 |
| Algae + DAC balanced | algae → carbon_capture → buffer | **9,894** (0 · 0 · 3683 · 0 · 6212) | 3,623 |
| Plastic recycling priority (MRF + enzymatic) | plastic_recycling → carbon_capture → buffer | **54,987** (0 · 0 · 0 · 54987 · 0) | -6,403 |
...
| Plastic recycling priority (MRF + enzymatic) | **~6,873 homes**, **~22.0 million shelter hot showers/yr (~60,260/day)** |
...
- **DAC priority (climate)** — **37,776 tonnes CO₂e/yr** net. **Robot priority:** carbon_capture → algae → buffer. **Routed heat:** **70,918 MWh/yr** (pools **0** · fisheries **0** · algae **0** · plastic **0** · DAC **70,918**). Pools and fisheries are not in the robot list — all recoverable heat goes to DAC regeneration (~71 GWh/yr). *Hypothetical redirect* (same MWh to community uses): **~8,865 homes** heat, **~28.4 million shelter hot showers/yr (~77,718/day)**.
- **Community heat (pools + fisheries)** — **1,721 tonnes CO₂e/yr** net. **Robot priority:** pool → aquaculture → algae → carbon_capture → buffer. **Routed heat:** **6,510 MWh/yr** (pools **97** · fisheries **104** · algae **3,409** · plastic **0** · DAC **2,900**). Amenities first; DAC gets leftover heat (~2.9 GWh/yr) — large CO₂ trade-off vs climate priority. Pool service: **0.5 olympic-pool equivalents** (2.2 community pools). Fisheries: **0.4 raceways** (500 m³), **~5,393 kg fish/yr** potential. Algae: **0.19 ha** thermal service (from **3,409 MWh/yr** to ponds). *Hypothetical redirect* (same MWh to community uses): **~814 homes** heat, **~2.6 million shelter hot showers/yr (~7,134/day)**.
- **Algae + DAC balanced** — **3,623 tonnes CO₂e/yr** net. **Robot priority:** algae → carbon_capture → buffer. **Routed heat:** **9,894 MWh/yr** (pools **0** · fisheries **0** · algae **3,683** · plastic **0** · DAC **6,212**). No pools/fisheries. Algae is first in line — daytime pond maintenance starves DAC (~6.2 GWh/yr), so total routed heat (~9.9 GWh/yr) is far below DAC-only (~71 GWh/yr). Algae: **0.21 ha** thermal service (from **3,683 MWh/yr** to ponds). *Hypothetical redirect* (same MWh to community uses): **~1,237 homes** heat, **~4.0 million shelter hot showers/yr (~10,843/day)**.
- **Plastic recycling priority (MRF + enzymatic)** — **-6,403 tonnes CO₂e/yr** net. **Robot priority:** plastic_recycling → carbon_capture → buffer. **Routed heat:** **54,987 MWh/yr** (pools **0** · fisheries **0** · algae **0** · plastic **54,987** · DAC **0**). Direct wash/enzyme + boosted hot wash first; DAC gets leftover heat. Plastic recycling: **~357,417 tonnes PET/yr** thermal-service equivalent (from **54,987 MWh/yr**). *Hypothetical redirect* (same MWh to community uses): **~6,873 homes** heat, **~22.0 million shelter hot showers/yr (~60,260/day)**.
...
**Nobody has built a Colossus-class MRF on a GPU hall yet.** This module is labeled speculative, like [Chimney DAC](#convection-speculative). It models a colocated **material recovery + enzymatic PET** plant with two thermal sub-loads: direct buffer heat to **65 °C** tanks and heat-pump-boosted **85 °C** hot wash / pre-dry.
...
**Plastic-recycling priority run:** **54,987 MWh/yr** to plastic loads (**~357,417 tonnes PET/yr** thermal-service equivalent) · **0 MWh/yr** leftover to DAC · **-6,403 tonnes CO₂e/yr** net (grid scenario).

**Trade-off (plastic vs. DAC priority):** plastic routing delivers **~357,417 tonnes PET/yr** thermal equivalent but **~44,178 fewer tonnes CO₂e/yr** net removed than DAC-only.

Pyrolysis for hard-to-recycle films and mixed polyolefins belongs in the same **industrial symbiosis** picture — DC heat handles upstream drying; reactor duty stays internal; flue CO₂ can route to the existing DAC model. See [`config/nvidia_us_plastic_recycling.yaml`](config/nvidia_us_plastic_recycling.yaml) and [`config/thermal_grades.yaml`](config/thermal_grades.yaml).
...
### Generated at: 2026-06-12T06:06:23.586262Z
...
### Acoustic side gig — fan orchestra, MSE, MDMG

Turn GPU fan noise into structured music — in simulation. Three layers:

1. **Fan orchestra** — 15,000 Pickaso-style bowed strings (1 per fan)
2. **MSE** — passive panels + water shape the fence-line sound
3. **MDMG** — spring networks refine toward music (mechanical diffusion)

Full walkthrough: [Acoustic section](#acoustic-speculative)

```bash
./gradlew generateAcousticFigures
```

Regenerates charts, audio clips, and the README section above.
...
## Acoustic side gig (speculative)

*A computer experiment — not mixed with main thermal CO₂ results. Nothing here is built at scale yet.*

### At a glance

**The question:** Data centers are loud because cooling fans never stop. What if that hum became structured music — using physics, not speakers?

**Jump to:** [Overview](#acoustic-overview) · [1 · Fan orchestra](#acoustic-fan-orchestra) · [2 · MSE equalizer](#acoustic-mse) · [3 · MDMG diffusion](#acoustic-mdmg) · [Audio](#acoustic-audio) · [Charts](#acoustic-charts) · [Research](#acoustic-research)

<a id="acoustic-overview"></a>

**Three layers — read top to bottom:**

| Step | Name | What it does (plain English) |
|------|------|------------------------------|
| **1** | **Fan orchestra** | Each cooling fan spins a tiny [Pickaso Rotary Bow](https://www.pickasobow.com/products/pickaso-rotary-bow) on a string — **15,000 sustained voices** at Colossus scale |
| **2** | **MSE** (Mechanical Soundscape Equalizer) | Honeycomb panels + water trickle **shape** the sound at the fence — no computer |
| **3** | **MDMG** (Mechanical Diffusion Music Generator) | Coupled springs **refine** the orchestra rumble toward smoother music — like AI diffusion, but in metal |

```
GPU fans  →  Fan orchestra (15k bowed strings)  →  MSE (panels + water)  →  Fence line
                              ↓
                         MDMG (spring refinement)
```

**Reference run headline numbers:**

- **15,000** bowed-string instruments (1 per fan)
- **78.85 dBA** at the fence before treatment
- **80.70 dBA** after MSE (decoupled)
- MDMG harmonicity **1.00** on fan-orchestra input

> Each GPU cooling fan spins a tiny rotary bow on a string — 15,000 of them turn the data center into a mechanical orchestra.

---

<a id="acoustic-fan-orchestra"></a>

### 1 · Fan orchestra — the music source

**The idea:** Mount a Pickaso-style rotary bow on every cooling fan. The fan shaft spins the bow wheel; the string never stops vibrating while GPUs run.

| Pickaso product (real) | Tremobow demo frame |
|------------------------|---------------------|
| ![Pickaso Rotary Bow](docs/images/pickaso_rotary_bow.jpg) | ![Tremobow](docs/images/pickaso_tremobow.jpg) |

Video: [Introducing the Pickaso Rotary Bow](https://www.youtube.com/watch?v=N7B5sxk9OlA)

**Pickaso → data center (one cell per fan):**

| On a guitar | On a cooling fan |
|-------------|------------------|
| Motor spins elastic bow wheel | Fan shaft drives bow wheel |
| Swappable covers (Tremobow, Vase, Curved) | Same three cover presets, rotated by voice |
| Endless sustain | Continuous bowing while fans run |
| One guitar | **15,000 cells** = 2,500 racks × 6 fans |

**Scale (from [`config/acoustic_spectrum.yaml`](config/acoustic_spectrum.yaml)):**

- **15,000** instruments · **~490 Hz** blade-passing tremolo · **150 m** fence distance

*One fan cell: shaft → bow wheel → string → fence*

![One fan cell: shaft → bow wheel → string → fence](docs/figures/fan_orchestra_cell.png)

*Scale: 1 Pickaso cell vs 15,000 fan-orchestra cells*

![Scale: 1 Pickaso cell vs 15,000 fan-orchestra cells](docs/figures/fan_orchestra_at_scale.png)

**Orchestra metrics (this run):**

| Metric | Value | In plain English |
|--------|-------|------------------|
| Active instruments | **15,000** | One bow cell per fan |
| Musical content | **69.44 dB** | Energy in bow partials (200 Hz–2 kHz) |
| Sustain index | **0.42** | How "bow-like" the waveform is (0–1) |
| Tremolo depth | **10.00 dB** | Blade-passing pulse on bow pressure |

<details>
<summary><strong>What if only some fans had bow cells?</strong></summary>

| % fans instrumented | Fence SPL | Tonal prominence |
|---------------------|-----------|------------------|
| 10.0% fans instrumented | **80.65 dBA** | **0.47 dB** |
| 50.0% fans instrumented | **80.66 dBA** | **0.56 dB** |
| 100.0% fans instrumented | **80.70 dBA** | **0.69 dB** |

</details>

<details>
<summary><strong>What if the hall were smaller or larger?</strong></summary>

| Racks | Fence SPL | Tonal prominence |
|-------|-----------|------------------|
| 500 racks | **79.79 dBA** | **0.72 dB** |
| 2500 racks | **80.70 dBA** | **0.69 dB** |
| 5000 racks | **81.62 dBA** | **0.66 dB** |

</details>

<details>
<summary><strong>Which Pickaso cover sounds brightest?</strong></summary>

| Cover | Fence SPL | Tonal prominence |
|-------|-----------|------------------|
| tremobow cover | **80.73 dBA** | **0.77 dB** |
| vase cover | **80.68 dBA** | **0.61 dB** |
| curved cover | **80.70 dBA** | **0.68 dB** |
...
### 2 · MSE — Mechanical Soundscape Equalizer

**What it does:** Passive hardware at the fence line — no AI, no speakers.

- **Metamaterial liner** — thin honeycomb panels that absorb the harshest fan tones (costs fan power)
- **Water soundscape** — warm exhaust pushes water over rocks for stream-like masking
- **Organ pipes** (optional) — chimney draft blows across tuned pipes

The fan-orchestra waveform is mixed in **first**, then liners and water shape the result.

**Results:**

| | Baseline (fans only) | MSE decoupled | MSE + chimney coupling |
|---|------------------------|---------------|------------------------|
| Fence SPL | **78.85 dBA** | **80.70 dBA** | **84.83 dBA** |
| Soundscape quality | — | **1.00** | see chart |
| Extra fan power (liner) | — | **184.62 kW** | — |

> **Verdict:** Metamaterial rack liners are real (Bell Labs ~2.5 dBA). Water masking improves *perceived* peace. Open questions: fan pressure drop, infrasound, campus scale-up.

*MSE: fence SPL vs metamaterial liner depth*

![MSE: fence SPL vs metamaterial liner depth](docs/figures/acoustic_spl_vs_liner_depth.png)

*MSE: soundscape quality vs water flow*

![MSE: soundscape quality vs water flow](docs/figures/acoustic_sqi_vs_water_flow.png)

*Baseline vs MSE decoupled vs chimney-coupled*

![Baseline vs MSE decoupled vs chimney-coupled](docs/figures/acoustic_coupling_comparison.png)

---

<a id="acoustic-mdmg"></a>

### 3 · MDMG — Mechanical Diffusion Music Generator

**What it does:** Refines the **fan-orchestra waveform** toward smoother music using coupled springs — a mechanical cartoon of AI diffusion (Langevin dynamics instead of a neural network).

| Software diffusion | Our MDMG cartoon |
|--------------------|------------------|
| Start from noise | Start from fan-orchestra clip |
| Neural denoiser | Fixed spring energy landscape |
...
| Metric | Value |
|--------|-------|
| Input | Fan-orchestra aggregate (15k voices) |
| Reverse steps | **40** |
| Spectral distance | **41.32** |
| Harmonicity | **1.00** |

For tier comparison (MDMG v1 / v2 / Java-LDM), see the [MDMG benchmark](#mdmg-benchmark) section below.

> **Verdict:** Speculative — no one has built campus-scale mechanical music hardware. This sim shows inference physics only.

*MDMG: quality vs reverse denoising steps*

![MDMG: quality vs reverse denoising steps](docs/figures/acoustic_mdmg_vs_steps.png)

---

<a id="acoustic-audio"></a>

### Listen — synthetic audio clips

**Step 1 — Fan orchestra**
- [fan_orchestra_15k.wav](docs/audio/fan_orchestra_15k.wav) — 15,000 bowed strings (aggregate)
- [fan_orchestra_vs_raw_fan.wav](docs/audio/fan_orchestra_vs_raw_fan.wav) — Orchestra minus raw fan bed
- [fan_noise_baseline.wav](docs/audio/fan_noise_baseline.wav) — Raw fan noise at fence (before treatment)

**Step 2 — MSE**
- [mse_perimeter_decoupled.wav](docs/audio/mse_perimeter_decoupled.wav) — Orchestra + metamaterial + water (decoupled)
- [mse_perimeter_coupled.wav](docs/audio/mse_perimeter_coupled.wav) — Same + chimney-draft coupling

**Step 3 — MDMG**
- [mdmg_output.wav](docs/audio/mdmg_output.wav) — MDMG refines fan-orchestra input

---

<a id="acoustic-charts"></a>

### Charts & sweeps

All figures regenerate with `./gradlew generateAcousticFigures`.
...
*More instrumented fans → more tonal bow energy*

![More instrumented fans → more tonal bow energy](docs/figures/acoustic_orchestra_instrumented_fraction.png)

*Bigger hall → louder fence line*

![Bigger hall → louder fence line](docs/figures/acoustic_orchestra_rack_count.png)

*Pickaso cover presets vs tonal prominence*

![Pickaso cover presets vs tonal prominence](docs/figures/acoustic_orchestra_bow_covers.png)

*Pickaso cover presets vs fence SPL*

![Pickaso cover presets vs fence SPL](docs/figures/acoustic_orchestra_bow_covers_spl.png)
...
<a id="acoustic-research"></a>

### Research, limits & verdicts
...
**How our numbers compare to published work:**

| Topic | This sim | Literature |
|-------|----------|------------|
| Metamaterial liner | **-1.85 dBA** shift | ~2.5 dBA (Bell Labs rack liner) |
| Water masking | SQI **1.00** | Informational masking (Galbrun & Ali 2013) |
| MDMG harmonicity | **1.00** | Lab-scale physical computing only |
...
**Relationship to the main repo:** Thermal side gigs route *heat* to DAC, algae, and recycling. This acoustic module asks what *sound* could become — complementary, not a replacement.
...
- Config: `config/acoustic_spectrum.yaml`, `config/mechanical_equalizer.yaml`, `config/mechanical_diffusion.yaml`
- Literature: `config/acoustic_references.yaml`
- Generated: 2026-06-10T00:19:47.044404Z
...
<p align="center">
  ...
  <strong>Job 1:</strong> power AI. <strong>Side gig:</strong> put the exhaust to work before it's thrown away.
</p>
